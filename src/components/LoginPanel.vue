...
<template>
  <div  class="bg-white text-black z-999 absolute w-[250px] rounded-2xl">
    ...
    >
      ...
      <a class="text-xs text-[#2b2b2b] text-center text-pretty" @click="()=>{resetPasswordPanelOpened=true;loginPanelOpened=false;}">Password dimenticata? Clicca qui</a>
      <button
      ...
      <p v-if="errors !== undefined" class="text-center font-fanwood text-red-800 font-bold">{{ errors.issues[0].message }}</p>
      <p v-if="session !== undefined && session.result === false" class="text-center font-fanwood text-red-800 font-bold">{{ session.description }}</p>
    </div>
  </div>
</template>
